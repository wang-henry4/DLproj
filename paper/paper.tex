...
What is more, we predict the classes based on the threshold that the probabilities of chosen classes have to be larger than twice of the probabilities of unchosen classes and the F1 score of the best model is 0.32. The data is very unbalanced, with the largest class counting for 42\% of the data and getting a F1 score of 0.680. So we choose the weighted F1 score. We noticed that our model performance is not good compared to that of HPA Cell Atlas team's study. It is possibly caused by the unbalance in data and limited number of trained epochs.
